...
{
  __shared__ int dev_as[BLOCK * BLOCK];
  __shared__ int dev_bs[BLOCK * BLOCK];
  ...
  for(m=0; m < N/BLOCK; m++){
      dev_as[ty * BLOCK + tx] = inMatA[row * N +(m * BLOCK + tx)];
      dev_bs[ty * BLOCK + tx] = inMatB[(m * BLOCK + ty) * N +col];
      __syncthreads();

      // 行列の演算を行う
      for(k = 0; k < BLOCK; k++){
          target += dev_as[ty * BLOCK + k] * dev_bs[k * BLOCK + tx];
      }
      __syncthreads();
  }

  inMatC[row * N + col] = target;
}
...
int main(int argc, char** argv)
{
    ...
    /* CPU側での処理時間計測 */
    start = cpuSecond();
#pragma omp parallel for private(scan,col,row)
    for (row = 0; row < N; row++) {
        for (col = 0; col < N; col++) {
